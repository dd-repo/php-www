...
foreach( $users as $u )
{
	if( $u['billing'] > 0 )
	{
		...
		$do = false;
		if( $u['billing'] == 1 && $bills[0]['date'] < strtotime('this day previous month') )
			$do = true;
		if( $u['billing'] == 2 && $bills[0]['date'] < strtotime('this day previous month') )
			$do = true;		
		if( $u['billing'] == 3 && $bills[0]['date'] < strtotime('this day previous year') )
			$do = true;
		...
		if( $do )
		{
			...
			if( $u['billing'] == 3 )
			{
				$bill = api::send('bill/insert', array('user'=>$u['id'], 'type'=>3), $GLOBALS['CONFIG']['API_USERNAME'].':'.$GLOBALS['CONFIG']['API_PASSWORD']);
				...
				for( $i = 1; $i <= 12; $i++ )
				{
					if( $ramPrice['price'] > 0 )
					{
						...
						api::send('bill/insertline', array('bill'=>$bill['id'], 'name'=>$ramPrice['name'], 'description'=>$ramPrice['desc'], 'amount'=>$ramPrice['price'], 'vat'=>$u['vat']), $GLOBALS['CONFIG']['API_USERNAME'].':'.$GLOBALS['CONFIG']['API_PASSWORD']);
					
						if( $diskPrice['price'] > 0 )
							api::send('bill/insertline', array('user'=>$u['id'], 'bill'=>$bill['id'], 'name'=>$diskPrice['name'], 'description'=>$diskPrice['desc'], 'amount'=>$diskPrice['price'], 'vat'=>$u['vat']), $GLOBALS['CONFIG']['API_USERNAME'].':'.$GLOBALS['CONFIG']['API_PASSWORD']);	
					}
					else if( $appsPrice['price'] > 0 )
					{
						$diskPrice = computeHostingDisk($disk);
						api::send('bill/insertline', array('bill'=>$bill['id'], 'name'=>$appsPrice['name'], 'description'=>$appsPrice['desc'], 'amount'=>$appsPrice['price'], 'vat'=>$u['vat']), $GLOBALS['CONFIG']['API_USERNAME'].':'.$GLOBALS['CONFIG']['API_PASSWORD']);
		...
		<br />
		Nous vous informons que votre facture est disponible dans votre <a style='color: #56c8f9; text-decoration: none;' href='https://www.anotherservice.com/panel/billing'>interface de gestion</a> Another Service. 
		...
		<br />
		Nous vous informons que votre facture est disponible dans votre <a style='color: #56c8f9; text-decoration: none;' href='https://www.anotherservice.com/panel/billing'>interface de gestion</a> Another Service. 
		Vous pouvez la <a style='color: #56c8f9; text-decoration: none;' href='https://www.anotherservice.com/panel/billing/view?id={BILL}'>payer directement</a> par carte bancaire.<br /><br /> Si vous avez souscrit au prélèvement automatique, vous pouvez ignorer cet email et votre facture sera marquée payée une fois le prélèvement effectué.<br /><br />
		Vous pouvez retrouver la facture dans votre interface à l'adresse : <a style='color: #56c8f9; text-decoration: none;' href='https://www.anotherservice.com/panel/billing/view?id={BILL}'>https://www.anotherservice.com/panel/billing/view?id={BILL}</a><br /><br />
		Nous vous remercions de votre confiance.<br /><br />
		Cordialement,<br />
		L'équipe Another Service";
